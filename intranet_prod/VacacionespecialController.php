...
class VacacionController extends ControllerBase
{
    ...
    /**
     * Validar condición de vacaciones truncas y que no se mezclen periodos 322
     */
    private function _validarCondicionTruncas($idSolicitante, $condicion, $postData)
{
    ...
    $porProgramar = isset($consolidado['por_programar']) ? floatval($consolidado['por_programar']) : 0;
    $programadosPrevios = isset($postData['programadosPrevios']) ? floatval($postData['programadosPrevios']) : $programado;

    // Validar que no se seleccione vacaciones truncas si hay vencidas o ganadas
    if ($condicion == '2') {
        $pendientesPrevios = max(0, $diasVencidos + $diasGanados - $programadosPrevios);
    ...
    // Validar que la cantidad de días solicitada no exceda el disponible
    ...
    if ($condicion == '1') {
        // Si existe por_programar, usarlo directamente (es más preciso)
        if ($porProgramar > 0) {
            $disponiblesPeriodo = $porProgramar;
            $tipoVacacion = 'pendientes por programar';
        } else {
            // Nueva lógica: primero vencidas, luego ganadas
            $diasDisponiblesVencido = 0;
            $diasDisponiblesGanado = 0;
            
            // Primero restamos programado de vencido
            if ($programado <= $diasVencidos) {
                $diasDisponiblesVencido = $diasVencidos - $programado;
                $diasDisponiblesGanado = $diasGanados;
            } else {
                // Si programado > vencido, el restante va a ganado
                $diasDisponiblesVencido = 0;
                $programadoRestante = $programado - $diasVencidos;
                $diasDisponiblesGanado = max(0, $diasGanados - $programadoRestante);
            }
            
            // Total disponible es la suma de ambos
            $disponiblesPeriodo = $diasDisponiblesVencido + $diasDisponiblesGanado;
            
            // Verificar de qué tipo son las vacaciones a tomar
            if ($cantidadDias <= $diasDisponiblesVencido) {
                $tipoVacacion = 'vencidas';
            } else if ($cantidadDias <= $disponiblesPeriodo) {
                $tipoVacacion = 'vencidas y pendientes';
                
                // Mensaje personalizado indicando cuántos días se tomarán de cada tipo
                $diasDeVencidas = min($cantidadDias, $diasDisponiblesVencido);
                $diasDeGanadas = $cantidadDias - $diasDeVencidas;
                
                if ($diasDeVencidas > 0 && $diasDeGanadas > 0) {
                    $mensajeDetalle = sprintf(
                        "Se tomarán %.2f días de vacaciones vencidas y %.2f días de vacaciones pendientes.", 
                        $diasDeVencidas, 
                        $diasDeGanadas
                    );
                }
        ...
        $disponiblesPeriodo = $diasTruncos;
    ...
    if ($cantidadDias > $disponiblesPeriodo) {
        $result['status'] = false;
        $mensaje = sprintf(
            "Solo tienes disponible %.2f días en el periodo de vacaciones %s", 
            $disponiblesPeriodo, 
            $tipoVacacion
        );
        
        // Si hay vacaciones truncas disponibles, sugerir usarlas
        if ($diasTruncos > 0 && $condicion == '1') {
    ...
    } else if (!empty($mensajeDetalle)) {
        // Añadir mensaje informativo si existe
        $result['mensajeInfo'] = $mensajeDetalle;
    }
